...
export default function Register() {
    ...
    const handleSubmite = (e)=>{
        ...
        fetch("http://localhost:5000/api/register",{headers:{"Content-Type":"application/x-www-form-urlencoded"},method:"POST",
        body:
        `username=${fields.username.value}&password=${fields.password.value}&phone=${fields.phone.value}&email=${fields.email.value}`
        })
        .then((r)=>{
            console.log(r)
            if( r.status == 200){
                e.target.querySelector(".form-error").setAttribute("show","false")
                let userName = atob(document.cookie.auth)?.username
                setState({isAuth:true,userName:userName});
            }else{
